  ...
  return (
    <section className="space-y-8">
      ...
      <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
        <MetricCard
          label="Page Views"
          value={String(chartData[0]?.count || 0)}
        />
        <MetricCard
          label="Add to Cart"
          value={String(
            chartData.find((c: any) => c.step === "add_to_cart")?.count || 0,
          )}
        />
        <MetricCard
          label="Checkout"
          value={String(
            chartData.find((c: any) => c.step === "begin_checkout")?.count || 0,
          )}
        />
        <MetricCard
          label="Purchases"
          value={String(
            chartData.find((c: any) => c.step === "purchase")?.count || 0,
          )}
        />
      </div>
          ...
          {data.funnel.dropoffRates.map((d: any, i: number) => (
            ...
            >
              ...
              <div className="flex items-center gap-4">
                <span>{d.count}</span>
                {i > 0 && (
                  <span className="text-[#EDE9E0]/45">
                    ↓ {d.dropoffRate.toFixed(1)}% drop-off
                  </span>
                )}
              </div>
            </div>
      ...
      <article className="min-w-0 border border-[#EDE9E0]/15 bg-[#080808] p-4">
        <div className="flex items-center justify-between">
          ...
          <p className="text-xl font-medium text-[#EDE9E0]">
            {data.cartAbandonment.rate.toFixed(1)}%
          </p>
        </div>
        <p className="mt-2 text-xs uppercase tracking-[0.12em] text-[#EDE9E0]/55">
          {data.cartAbandonment.abandoners} abandoners out of{" "}
          {data.cartAbandonment.totalCartUsers} cart users
        </p>
            ...
            <table className="w-full text-left text-xs">
              <thead>
                <tr className="border-b border-[#EDE9E0]/15">
                  <th className="pb-2 uppercase tracking-[0.12em] text-[#EDE9E0]/55">
                    User
                  </th>
                  <th className="pb-2 uppercase tracking-[0.12em] text-[#EDE9E0]/55">
                    Page
              ...
              <tbody>
                {data.cartAbandonment.recentAbandoners
                  .slice(0, 20)
                  .map((a: any, i: number) => (
                    <tr key={i} className="border-b border-[#EDE9E0]/10">
                      <td className="py-2 text-[#EDE9E0]/70">
                        {a.userId ||
                          `anon...${a.sessionId?.slice(-6) || "unknown"}`}
                      </td>
                      <td className="py-2 text-[#EDE9E0]/60">
                        {a.page || "-"}
                      </td>
                      <td className="py-2 text-[#EDE9E0]/60">
                        {a.value ? `${a.value} EGP` : "-"}
                      </td>
                      <td className="py-2 text-[#EDE9E0]/50">
