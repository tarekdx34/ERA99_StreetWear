...
import { products as allProducts, getProductBySlug } from '@/lib/products'
import Link from 'next/link'

const tickerText =
  'FREE DELIVERY IN ALEX - قطب - FIRST DROP LIVE NOW - HEAVYWEIGHT BOXY FIT - قطب - ALEXANDRIA'
...
// Use first 4 products from lib/products for the landing grid
const landingProducts = allProducts.slice(0, 4)

const instagramTiles = ['/images/1.jpeg', '/images/2.jpeg', '/images/3.jpeg', '/images/4.png', '/images/5.jpeg', '/images/6.jpeg']
    ...
    >
      <div className="mx-auto grid h-16 max-w-7xl grid-cols-[1fr_auto_1fr] items-center px-6 md:px-10">
        ...
        <div className="flex items-center justify-self-end gap-3 text-ash">
          <button aria-label="Search" className="border border-ash/30 p-2 hover:border-ash">
            <Search size={16} strokeWidth={1.7} />
          </button>
          <button aria-label="Cart" className="border border-ash/30 p-2 hover:border-ash">
...
function HeroBanner() {
  return (
    <section className="relative h-screen w-screen overflow-hidden">
      <img src={heroImage} alt="QUTB campaign" className="h-full w-full object-cover" />
  ...
  return (
    <section className="grid w-full grid-cols-1 md:grid-cols-3">
      {bannerPanels.map((panel) => (
        <motion.article key={panel.title} whileHover={{ scale: 1.01 }} className="relative aspect-[3/4] overflow-hidden">
          <img src={panel.image} alt={panel.title} className="h-full w-full object-cover" />
          <div className="absolute inset-0 bg-black/45" />
          <div className="absolute inset-0 flex flex-col items-center justify-center text-center">
            <h3 className="font-sans text-3xl font-medium uppercase tracking-[0.16em] text-ash">{panel.title}</h3>
            <a href="#drop" className="mt-3 text-xs uppercase tracking-[0.2em] text-ash hover:text-[#8B0000]">
              SHOP NOW →
            </a>
          </div>
        </motion.article>
...
function ProductCard({ product }: { product: (typeof allProducts)[0] }) {
  ...
  return (
    ...
    >
      ...
      <motion.div
        animate={{ opacity: hovered ? 1 : 0, height: hovered ? 'auto' : 0, y: hovered ? 0 : 8 }}
        transition={{ duration: 0.3, ease: 'easeOut' }}
  ...
  return (
    <section id="drop" className="bg-ink px-6 py-20 md:px-10">
      <div className="mx-auto max-w-7xl">
        <h2 className="font-sans text-5xl font-medium uppercase tracking-[0.08em] text-ash md:text-6xl">THE DROP</h2>
        <p className="mt-2 text-xs uppercase tracking-[0.24em] text-ash/55">DROP 001 - ALEXANDRIA</p>
...
function BrandStatement() {
  return (
    <section id="statement" className="relative w-full overflow-hidden bg-ink px-6 py-24 md:px-10">
      ...
      <div className="mx-auto grid max-w-7xl grid-cols-1 gap-10 md:grid-cols-2">
        ...
        <div className="relative z-10 flex flex-col justify-center">
          <p className="text-xs uppercase tracking-[0.22em] text-ash/50">WHO WE ARE</p>
          <h3 className="mt-5 max-w-2xl text-3xl leading-[1.3] text-ash md:text-[32px]">
            We are the axis. The fixed point. Everything revolves - we stay still.
          </h3>
          <p className="mt-5 max-w-xl text-base text-ash/60">
            Born in Alexandria. Built for those who do not ask for permission.
          </p>
          <Link href="/story" className="mt-8 w-fit text-sm uppercase tracking-[0.16em] text-ash hover:underline">
            READ THE STORY →
          </Link>
        </div>
      </div>
    ...
    <section className="w-full bg-[#8B0000] px-6 py-6 text-ash md:px-10">
      <div className="mx-auto flex max-w-7xl flex-col items-center justify-between gap-4 md:flex-row">
        <p className="text-center text-xs uppercase tracking-[0.2em] md:text-left">
          DROP 001 - LIMITED QUANTITIES - ORDER NOW
        </p>
        <div className="flex items-center gap-2 text-xs uppercase tracking-[0.15em]">
          <div className="border border-ash/40 bg-black/20 px-2 py-1">{format(time.days)}D</div>
          <span>:</span>
          <div className="border border-ash/40 bg-black/20 px-2 py-1">{format(time.hours)}H</div>
          <span>:</span>
          <div className="border border-ash/40 bg-black/20 px-2 py-1">{format(time.minutes)}M</div>
          <span>:</span>
          <div className="border border-ash/40 bg-black/20 px-2 py-1">{format(time.seconds)}S</div>
        </div>
      </div>
    </section>
  )
...
function InstagramStrip() {
  return (
    <section className="bg-ink px-6 py-20 md:px-10">
      <div className="mx-auto max-w-7xl">
        <h2 className="font-blackletter text-5xl text-ash md:text-6xl">@QUTBCO</h2>
        <div className="mt-8 grid grid-cols-1 gap-3 md:grid-cols-3 xl:grid-cols-6">
          {instagramTiles.map((image, index) => (
            ...
            >
              <img src={image} alt={`Instagram tile ${index + 1}`} className="h-full w-full object-cover" />
              <motion.div
        ...
        <a href="#" className="mt-8 inline-block text-sm uppercase tracking-[0.18em] text-ash hover:underline">
    ...
    <footer id="footer" className="bg-ink px-6 pb-8 pt-16 md:px-10">
      <div className="mx-auto grid max-w-7xl grid-cols-1 gap-10 border-t border-ash/20 pt-12 md:grid-cols-3">
        <div>
          ...
          <p className="mt-3 text-xs uppercase tracking-[0.16em] text-ash/65">Alexandria, Egypt</p>
        ...
        <div>
          <p className="text-xs uppercase tracking-[0.2em] text-ash/70">JOIN THE DROP</p>
          <form className="mt-4 flex w-full max-w-sm">
            ...
            />
            <button type="submit" className="border border-l-0 border-ash/25 bg-ash px-4 py-3 text-xs uppercase tracking-[0.12em] text-black">
              JOIN
            </button>
          </form>
        </div>
