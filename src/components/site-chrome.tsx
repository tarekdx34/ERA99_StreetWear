...
function Logo() {
  return (
    <span className="inline-flex items-end text-[34px] leading-none text-ash">
      ...
      <span className="font-sans text-[0.82em] uppercase tracking-[0.12em]">UTB</span>
    </span>
  );
}
...
export function SiteChrome() {
  ...
  return (
    <>
      ...
      <header className="fixed left-0 top-8 z-50 w-full border-b border-ash/20 bg-transparent">
        <div className="mx-auto grid h-16 w-full max-w-7xl grid-cols-[1fr_auto_1fr] items-center px-6 md:px-10">
          ...
          <nav className="hidden items-center gap-10 text-xs font-medium tracking-[0.22em] text-ash md:flex">
            <Link href="/#drop" className="transition-colors hover:text-concrete">
              SHOP
            </Link>
            <Link href="/story" className="transition-colors hover:text-concrete">
              STORY
            </Link>
            <Link href="/#footer" className="transition-colors hover:text-concrete">
              ALEX
            </Link>
          </nav>
          <div className="flex items-center justify-self-end gap-2 text-ash">
            <button aria-label="Search" className="grid h-10 w-10 place-items-center border border-ash/35 hover:border-ash">
              <SearchIcon />
            </button>
            <button
              aria-label="Cart"
