...
async function getEarlyAccessActiveFromInternalState(request: NextRequest) {
  try {
    const origin = process.env.NEXT_PUBLIC_SITE_URL || request.nextUrl.origin || "http://localhost:3250";
    const stateUrl = new URL("/api/internal/early-access-state", origin);
    const response = await fetch(stateUrl.toString(), { cache: "no-store", headers: { "cookie": request.headers.get("cookie") || "" } });
    if (!response.ok) {
      console.log("Internal state fetch failed:", response.status);
      return false;
    }
    const data = (await response.json().catch(() => null)) as
      | { earlyAccessActive?: unknown; countdownEnabled?: unknown }
      | null;
